...
<template>
  <div>
    <h1 class="text-h5 font-weight-bold mb-6">{{ $t('Dashboard') }}</h1>
      ...
      <VCol cols="12" sm="6" lg="3">
        <VCard>
          <VCardText class="d-flex align-center gap-3">
            ...
            <div>
              <div class="text-caption text-medium-emphasis">{{ $t('Total Users') }}</div>
        ...
        <VCard :title="$t('Recent Users')">
          ...
          <VCardActions>
            <RouterLink :to="{ name: 'user-page' }">
              <VBtn variant="text" size="small">{{ $t('View all users') }}</VBtn>
            </RouterLink>
          </VCardActions>
        </VCard>
      </VCol>
